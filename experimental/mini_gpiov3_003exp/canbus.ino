...
void obd_response(byte therequest)
{
  ...
  uint16_t O2_1v;     // data *100
  int16_t O2_1pc;     // data *10
  O2_1v = 111;//(realtimebufferA[24]*100);
  O2_1pc = 273;//realtimebufferA[10]*10;
  obdcalcA = O2_1v *2;
  ...
  uint16_t O2_2v;       //data *100
  int16_t O2_2pc;       //data *10
  O2_2v = 125;   //realtimebufferA[24]*100;
  O2_2pc = 264;  //realtimebufferA[39]*10;
  obdcalcA = O2_2v *2;
  ...
  uint16_t O2_1e;
  int16_t O2_1V; 
  O2_1e = (realtimebufferA[10]*10) / realtimebufferA[81];     //afr / stoich
  O2_1V = (realtimebufferA[10]*10);                             //afr
  obdcalcA = ((O2_1e)*65536)/200;
  obdcalcE = obdcalcA/256;              //the rmainder will be lost for the next calculation which we dont want! it is the first byte for sending
  obdcalcF = obdcalcE*256;              //
  obdcalcB = obdcalcA - obdcalcF;       //calcs the second byte for sending
  
  obdcalcC = ((O2_1V)*65536)/80;
  obdcalcE = obdcalcC/256;              //the rmainder will be lost for the next calculation which we dont want! it is the first byte for sending
  obdcalcF = obdcalcE*256;              //
  obdcalcD = obdcalcC - obdcalcF;       //calcs the second byte for sending
  ...
  uint16_t O2_2e;
  int16_t O2_2V; 
  O2_2e = (realtimebufferA[39]*10) / realtimebufferA[81];   //afr2 / stoich
  O2_2V = (realtimebufferA[39]*10);                           //afr2
  obdcalcA = ((O2_2e)*65536)/200;
  obdcalcE = obdcalcA/256;              //the rmainder will be lost for the next calculation which we dont want! it is the first byte for sending
  obdcalcF = obdcalcE*256;              //
  obdcalcB = obdcalcA - obdcalcF;       //calcs the second byte for sending
  
  obdcalcC = ((O2_2V)*65536)/80;
  obdcalcE = obdcalcC/256;              //the rmainder will be lost for the next calculation which we dont want! it is the first byte for sending
  obdcalcF = obdcalcE*256;              //
  obdcalcD = obdcalcC - obdcalcF;       //calcs the second byte for sending
