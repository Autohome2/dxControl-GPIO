...
struct statuses {
 ...
 volatile uint16_t digOut;        //bits showing digital output state
 volatile uint16_t digOut_Active; // bits show if channel is used by board selected when pin value is > 0
 volatile uint16_t digIn;         // bits showing digital input state
 volatile uint16_t digIn_Active;  // bits showing channel is used by board selected when pin value is > 0
 volatile uint16_t Analog[16];    // 16bit analog value data array for local analog
 volatile uint16_t Analog_Active; // bits showing channel is used by board selected when pin value is > 0
 volatile uint16_t EXin[16];      // 16bit value data array for external analog
...
uint16_t exinsel;
uint16_t INdata_from_Can[16];    //can address of source of data 0x200(512dec) to 0x7FF(1047dec) as 0 dec - 535 dec
